...
class Sandbox {
  ...
  checkStatus(cb) {
    ...
    exec(ins, (error, stdout, stderr) => {
      if (error) {
        console.log("******************************************ERROR EJECUCION DOCKER SANDBOX*********************************")
        console.log(error)
        cb(false)
        return
      }
  ...
  createError(stderr, typeError) {
    console.log("*****************************ENTRA AL LOG DE ERROR**********************************");
    let fileName = this.file_name.split('.')[0]
    let extension = typeError == 0 ? '.out' : '.err'
    let regex = new RegExp('/files/' + this.execution_directory.split('/')[5] + '/', 'g');
    let error = '\n' + stderr.replace(regex, '')
    
    fs.writeFile('/usr/src/app/files/errors/' + fileName + extension, error, (err) => {
